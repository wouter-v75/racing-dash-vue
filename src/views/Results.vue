<template>
  <div class="results-container">
    ...
    <main class="results-main">
      ...
      <!-- Race 2 Section with FLIP CARD -->
      <section class="latest-race-section">
        <h2 class="section-title">🏁 RACE 2 RESULTS</h2>
        <p class="section-subtitle">Race 2 • 10/09/2024 • CLICK CARD TO FLIP</p>
        
        <!-- RACE 2 FLIP CARD -->
        <div class="race-flip-card-container" @click="testRace2Flip">
          <div class="race-flip-card-inner" :class="{ 'flipped': race2CardIsFlipped }">
            
            <!-- RACE 2 FRONT SIDE - 5 Metrics -->
            <div class="race-flip-card-front">
              <div class="race-card-header">
                <h3>🏁 Race 2 - NORTHSTAR Performance</h3>
                <div class="race-flip-indicators">
                  <span class="race-date">10/09/2024</span>
              ...
              <div class="race-stats-5-metric">
                <div class="metric-item">
                  <span class="metric-label">Position</span>
                  <span class="metric-value position-color">
                    {{ northstarRace2Result?.position || '2nd' }}
                  </span>
                </div>
                ...
                <div class="metric-item">
                  <span class="metric-label">Finish Time</span>
                  <span class="metric-value time-color">
                    {{ northstarRace2Result?.finishTime || '–' }}
                  </span>
                </div>
                ...
                <div class="metric-item">
                  <span class="metric-label">Δ to 1st</span>
                  <span class="metric-value delta-color">
                    {{ northstarRace2Result?.deltaToFirst || '–' }}
                  </span>
                </div>
                ...
                <div class="metric-item">
                  <span class="metric-label">Δ to Ahead</span>
                  <span class="metric-value delta-color">
                    {{ northstarRace2Result?.deltaToAhead || '–' }}
                  </span>
                </div>
                ...
                <div class="metric-item">
                  <span class="metric-label">Δ to Behind</span>
                  <span class="metric-value delta-color">
                    {{ northstarRace2Result?.deltaToBehind || '–' }}
                  </span>
                </div>
              </div>
            ...
            <!-- RACE 2 BACK SIDE - Full Race Results Table -->
            <div class="race-flip-card-back">
              <div class="race-card-header">
                <h3>📊 Full Race 2 Results</h3>
                <div class="race-flip-indicators">
                  <span class="race-date">10/09/2024</span>
                  <span class="race-flip-indicator">{{ race2CardIsFlipped ? 'BACK' : 'FRONT' }}</span>
                </div>
              ...
              <div class="race-back-content">
                <div class="race-results-table">
                  ...
                  <div v-if="race2Results.length === 0" class="race-table-row">
                    <span style="grid-column: 1 / -1; text-align: center; opacity: 0.7; padding: 20px;">
                      {{ loadingRace2 ? 'Loading race data...' : 'No race data available' }}
                    </span>
                  </div>
                  
                  <div v-for="(boat, index) in race2Results.slice(0, 8)" 
                       :key="`race2-boat-${index}`"
                       class="race-table-row"
                       :class="{ 'highlight': findNorthstar(boat.name) }">
                    <span>{{ boat.position || '–' }}</span>
                    <span class="boat-name">{{ boat.name || 'Unknown' }}</span>
                    ...
                    <span class="race-delta" :class="{ 
                      ...
                      {{ formatDeltaToFirst(boat, race2Results) }}
                    </span>
                    <span class="race-status" :class="{ 
                      'finished': !/^(RET|DNF|DNS|DSQ|DNC)$/i.test(boat.finishTime || boat.elapsed || ''),
                      'retired': /^(RET|DNF|DNS|DSQ|DNC)$/i.test(boat.finishTime || boat.elapsed || '')
      ...
      <!-- Race 3 Section with FLIP CARD -->
      <section class="latest-race-section">
        <h2 class="section-title">🏁 RACE 3 RESULTS</h2>
        <p class="section-subtitle">Race 3 • 13/09/2024 • CLICK CARD TO FLIP</p>
        
        <!-- RACE 3 FLIP CARD -->
        <div class="race-flip-card-container" @click="testRace3Flip">
          <div class="race-flip-card-inner" :class="{ 'flipped': race3CardIsFlipped }">
            
            <!-- RACE 3 FRONT SIDE - 5 Metrics -->
            <div class="race-flip-card-front">
              <div class="race-card-header">
                <h3>🏁 Race 3 - NORTHSTAR Performance</h3>
                <div class="race-flip-indicators">
                  <span class="race-date">13/09/2024</span>
              ...
              <div class="race-stats-5-metric">
                <div class="metric-item">
                  <span class="metric-label">Position</span>
                  <span class="metric-value position-color">
                    {{ northstarRace3Result?.position || '1st' }}
                  </span>
                </div>
                ...
                <div class="metric-item">
                  <span class="metric-label">Finish Time</span>
                  <span class="metric-value time-color">
                    {{ northstarRace3Result?.finishTime || '–' }}
                  </span>
                </div>
                ...
                <div class="metric-item">
                  <span class="metric-label">Δ to 1st</span>
                  <span class="metric-value delta-color">
                    {{ northstarRace3Result?.deltaToFirst || '00:00' }}
                  </span>
                </div>
                ...
                <div class="metric-item">
                  <span class="metric-label">Δ to Ahead</span>
                  <span class="metric-value delta-color">
                    {{ northstarRace3Result?.deltaToAhead || '–' }}
                  </span>
                </div>
                ...
                <div class="metric-item">
                  <span class="metric-label">Δ to Behind</span>
                  <span class="metric-value delta-color">
                    {{ northstarRace3Result?.deltaToBehind || '–' }}
                  </span>
                </div>
              </div>
              ...
              <div class="race-back-content">
                <div class="race-results-table">
                  ...
                  <div v-if="race3Results.length === 0" class="race-table-row">
                    <span style="grid-column: 1 / -1; text-align: center; opacity: 0.7; padding: 20px;">
                      {{ loadingRace3 ? 'Loading race data...' : 'No race data available' }}
                    </span>
                  </div>
                  
                  <div v-for="(boat, index) in race3Results.slice(0, 8)" 
                       :key="`race3-boat-${index}`"
                       class="race-table-row"
                       :class="{ 'highlight': findNorthstar(boat.name) }">
                    <span>{{ boat.position || '–' }}</span>
                    <span class="boat-name">{{ boat.name || 'Unknown' }}</span>
                    ...
                    <span class="race-delta" :class="{ 
                      ...
                      {{ formatDeltaToFirst(boat, race3Results) }}
                    </span>
                    <span class="race-status" :class="{ 
                      'finished': !/^(RET|DNF|DNS|DSQ|DNC)$/i.test(boat.finishTime || boat.elapsed || ''),
                      'retired': /^(RET|DNF|DNS|DSQ|DNC)$/i.test(boat.finishTime || boat.elapsed || '')
