...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'classes') {
      ...
      console.log(`Found ${classes.length} classes for event ${eventId}:`, classes.map(c => c.id))
    ...
    if (type === 'overall') {
      ...
      console.log(`Parsed ${rows.length} overall results for class ${cls}`)
      if (rows.length > 0) {
        console.log('Sample boat names:', rows.slice(0, 3).map(r => r.name))
      }
    ...
    if (type === 'raceRaw') {
      ...
      console.log(`Parsed ${rows.length} race results for raceId ${raceId}, classId ${classId}`)
      if (rows.length > 0) {
        console.log('Sample boat names:', rows.slice(0, 3).map(r => r.name))
      }
...
function parseOverallByHeaders(htmlRaw) {
  ...
  const iPos     = headerIndex(headers, '^(pos|position|overall.*pos)$')
  const iBoat    = headerIndex(headers, '(boat|yacht.*name|name)')
  const iSail    = headerIndex(headers, '(sail.*no|sail|no)')
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    out.push({
      ...
      name:     grab(cells, iBoat, 2),  // Column 2: Yacht Name (not column 1 which is Nation)
      sailNo:   grab(cells, iSail, 3),  // Column 3: Sail No
      skipper:  grab(cells, iSkipper, 5), // Column 5: Owner
      points:   grab(cells, iPoints, 8),   // Adjust for actual points column
      total:    grab(cells, iTotal, 8) || grab(cells, iPoints, 8)
    })
  }
  return out
}
...
function toRaceRows(tableHtml) {
  ...
  const iBoat  = headerIndex(headers, '(boat|yacht.*name|name)')
  const iSail  = headerIndex(headers, '(sail.*no|sail|no)')
  const iOwner = headerIndex(headers, '(owner|skipper)')
  const iFinish= headerIndex(headers, 'finish.*time')
  ...
  for (let r=1;r<rows.length;r++){
    ...
    out.push({
      position:      grab(cells, iPos, 0),
      name:          grab(cells, iBoat, 2),  // Column 2: Yacht Name
      sailNo:        grab(cells, iSail, 3),  // Column 3: Sail No
      skipper:       grab(cells, iOwner, 5), // Column 5: Owner
      finishTime:    grab(cells, iFinish, 6), // Column 6: Finish Time
      elapsed:       grab(cells, iElaps, 7),  // Column 7: Elapsed
      correctedTime: grab(cells, iCorr, 8)    // Column 8: Corrected
    })
  }
  if (out.length) {
    const first = out[0].correctedTime
    for (const r of out) r.deltaToFirst = (first && r.correctedTime) ? 
      mmssDelta(first, r.correctedTime) : '–'
  }
  return out
}
